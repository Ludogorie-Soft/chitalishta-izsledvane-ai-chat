...
class TestClearIndex:
    """Tests for POST /index/clear endpoint."""

    def test_clear_index_removes_all_documents(
        self, test_indexing_app, seeded_test_data, monkeypatch
    ):
        """Test that clear endpoint removes all indexed documents."""
        test_client, test_indexing_service = test_indexing_app
        ...
        index_response = test_client.post("/index/database?limit=3")
        assert index_response.status_code == 200
        assert index_response.json()["indexed"] > 0

        # Verify documents exist
        stats_before = test_client.get("/index/stats").json()
        assert stats_before["total_documents"] > 0

        # Clear index
        clear_response = test_client.post("/index/clear")
        assert clear_response.status_code == 200
        assert clear_response.json()["status"] == "success"

        # Verify index is empty
        stats_after = test_client.get("/index/stats").json()
        assert stats_after["total_documents"] == 0
        assert stats_after["source_distribution"] == {}

    def test_clear_index_allows_re_indexing(
        self, test_indexing_app, seeded_test_data, monkeypatch
    ):
        """Test that after clearing, documents can be re-indexed."""
        test_client, test_indexing_service = test_indexing_app
        ...
        # Index documents
        index1 = test_client.post("/index/database?limit=2")
        assert index1.status_code == 200
        first_count = index1.json()["indexed"]
        ...
        # Verify cleared
        stats_after_clear = test_client.get("/index/stats").json()
        assert stats_after_clear["total_documents"] == 0

        # Re-index
        index2 = test_client.post("/index/database?limit=2")
        assert index2.status_code == 200
        second_count = index2.json()["indexed"]

        # Should be able to index again (may be same or different depending on data)
        assert second_count > 0

        # Verify documents are indexed
        stats_after_reindex = test_client.get("/index/stats").json()
        assert stats_after_reindex["total_documents"] == second_count

    def test_clear_index_empty_collection(
        self, test_indexing_app, monkeypatch
    ):
        ...
        response = test_client.post("/index/clear")
        assert response.status_code == 200
        assert response.json()["status"] == "success"

        # Verify still empty
        stats = test_client.get("/index/stats").json()
