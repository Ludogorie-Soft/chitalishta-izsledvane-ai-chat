...
def test_models():
    ...
    try:
        ...
        if first_chitalishte.region:
            filtered = db.query(Chitalishte).filter(
                Chitalishte.region == first_chitalishte.region
            ).count()
            print(f"[OK] Found {filtered} Chitalishte records in region '{first_chitalishte.region}'")
        
        # Test 7: Query InformationCards by year
        if info_cards and info_cards[0].year:
            year = info_cards[0].year
            cards_by_year = db.query(InformationCard).filter(
                InformationCard.year == year
            ).count()
        ...
        chitalishte_with_cards = db.query(Chitalishte).join(
            InformationCard
        ).filter(
            InformationCard.year.isnot(None)
        ).distinct().count()
